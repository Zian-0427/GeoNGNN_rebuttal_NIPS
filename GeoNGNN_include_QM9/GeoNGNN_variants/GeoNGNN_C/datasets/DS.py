...
class batched_chiral(Dataset):
    ...
    def raw_file_names(self):
        return ["ds/ds_raw.pt"] # A fake name. The raw data and process function are defined outside. See README.
    
    @property
    def processed_file_names(self):
        return ["ds/ds_full.pt"] # Also not used. The data is loaded outside.
    def download(self):
        pass
    def process(self):
        pass
...
def DS_datawork(
    ...
    **kwargs,
    ...
    data = torch.load(f"{root}/processed/ds/ds_full.pt")
    print("Finished.")
    ...
    train_num, val_num, test_num = 234622, 49878, 50571
    ...
    global_y_mean = torch.cat([data.y for data in all_dataset]).mean(dim=0).to(device=device)
    global_y_std = torch.cat([data.y for data in all_dataset]).std(dim=0).to(device=device)
